function [Lev_sub]=de_coef3dB_cell(bin,delta0)
...
w=2^4; nn=w*w*wh;
r=1; c=1;
[subr,ptr]=de_sub3d(bin{5,1},nn,delta0,n4,1); 
Lev_sub( (r-1)*w+1:r*w, (c-1)*w+1:c*w, :) = subr;

rc=[1 2; 2 2; 2 1];
sb=cell(1,3);
[sb{1},sb{2},sb{3},ptr]=de_sub3d3(bin{5,2},nn*3,delta0,n4,1);
for ia=1:3
   r=rc(ia,1); c=rc(ia,2);
...
for ia=1:3
   r=rc(ia,1); c=rc(ia,2);
   %row_col=[r c] %<==================================================
   [subr,ptr]=de_sub3d(bin{7,ia},nn,delta0,n6,1); 
   Lev_sub( (r-1)*w+1:r*w, (c-1)*w+1:c*w, :) = subr;
end
...
for ia=1:3
   r=rc(ia,1); c=rc(ia,2);
   %row_col=[r c] %<==================================================
   [subr,ptr]=de_sub3d(bin{8,ia},nn,delta0,n7,1); 
   Lev_sub( (r-1)*w+1:r*w, (c-1)*w+1:c*w, :) = subr;
end
