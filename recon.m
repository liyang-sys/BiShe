rate=0.125; n=length(rate);
%rate=[0.125 0.25 0.5 1 2 3]; n=length(rate);
SN=zeros(1,n);
...
for ia=1:n
   ...
   %coef=tree2block4bandC(coef,tanTheta);  qb=LFP2Inv(coef,4,fL);
   %coef=band2block(coef,4,tanTheta);  qb=LFP2Inv(coef,4,fL);
   %coef=tree2blockC(coef,tanTheta); qb=LFP2Inv(coef,8,fL); %<==== LFP
   ...
   %coef=tree2wp(coef,4,2,1); qb=swp2invModif(coef,hh,gg,hhh,ggg);
